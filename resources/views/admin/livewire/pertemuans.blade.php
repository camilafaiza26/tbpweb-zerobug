@extends('admin.navigation-menu')
                 ...
                 <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                    ...
                    <div class="flex mb-4 px-4">
                     ...
                     <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0 ">
                         <button  wire:click="showModal()" class="bg-indigo-700 hover:bg-indigo-900 text-white font-bold py-3 px-4 rounded mb-2 inline-flex items-center  transition duration-500 ease-in-out bg-indigo-600 hover:bg-indigo-900 transform hover:-translate-y-1 hover:scale-110">
                             ...
                             @if($isOpen)
                             @include('admin.livewire.createpert');
                                 @endif          
                     ...
                     <table class="table-auto w-full">
                         <thead style="background-color:#4723D9" class="bg-blue-500">
                             <tr>
                                 ...
                                 <th class="px-4 py-2 text-white w-auto">Materi</th>
                         ...
                         <tbody>
                             ...
                             <tr class="h-12 hover:bg-gray-100">          
                                 ...
                                 <td class="text-center">
                                    {{-- onclick="window.location.href='{{route('detail.pertemuan',$kelas_id->id)}}'" --}}
                                    <button onclick="window.location.href='/admin/kelas/detail/{{$pertemuan->kelas->id}}/pertemuan/absensi/{{$pertemuan->id}}'" class="bg-green-500 text-white font-bold py-1 px-4 rounded  transition duration-500 ease-in-out bg-green-600 hover:bg-green-500 transform hover:-translate-y-1 hover:scale-110 "> Detail</button>
                                    {{-- <button onclick="window.location.href='{{route('detail.pertemuan',$pertemuan->kelas->id, $pertemuan->id) }}'" class="bg-green-500 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded">
                                        Detail
                                     </button> --}}
                                     <button wire:click="edit({{ $pertemuan->id }})" class="bg-blue-500 text-white font-bold py-1 px-4 rounded  transition duration-500 ease-in-out bg-blue-600 hover:bg-blue-500 transform hover:-translate-y-1 hover:scale-110 ">
                                     Edit
                                     </button>
                                     <button wire:click="deleteConfirms({{ $pertemuan->id }})" class="bg-red-500 text-white font-bold py-1 px-4 rounded  transition duration-500 ease-in-out bg-red-600 hover:bg-red-500 transform hover:-translate-y-1 hover:scale-110 ">
                                     Hapus
                                     </button>
                                 </td>
                             </tr>
 ...
 <x-jet-dialog-modal wire:model="deleteConfirms">
    ...
    <x-slot name="content">
        {{ __('Apakah anda yakin menghapus Pertemuan ini?') }}
    </x-slot>
